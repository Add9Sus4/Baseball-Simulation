...
task :simulate_games => :environment do
  ...
  20.times do |index|
    ...
    @game_number = @season.next_game
    puts "game number: #{@game_number} (Day: #{@season.current_date})"

    # Find all games on this date
    @games = Game.where("sim_date = ?", @season.current_date)
    @games.each do |game|
      game.prepare(Team.find(game.home_team_id), Team.find(game.away_team_id), @game_number, 0)
      game.play
    end
